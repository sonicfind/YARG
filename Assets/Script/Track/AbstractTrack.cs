...
namespace YARG.PlayMode
{
    public abstract class AbstractTrack : MonoBehaviour
    {
        ...
        private int SoloHitPercent =>
            soloNoteCount > 0 ? Mathf.FloorToInt((float) soloNotesHit / soloNoteCount * 100f) : 0;
        ...
        protected int Combo
        {
            ...
            set
            {
                ...
                if (value == 0 && CurrentStarpower?.time <= HitMarginStartTime &&
                    CurrentNote?.time >= CurrentStarpower?.time)
                {
        ...
        protected bool Beat { get; private set; }
        ...
        private void Start()
        {
            ...
            const float
                baseOffset =
                    (Constants.HIT_MARGIN_FRONT - Constants.HIT_MARGIN_BACK) /
                    2; // Offsetting is done based on half of the size
            ...
            foreach (var eventInfo in Play.Instance.chart.events)
            {
                ...
                else if (eventInfo.name == soloName)
                {
                    ...
                    int noteCount = GetNoteCountForPhrase(eventInfo, out soloNoteIndex, chordsAreSingleNote,
                        soloNoteIndex);
                    if (noteCount > 0)
                    {
            ...
            if (commonTrack.noteStreakNotifsEnabled)
            {
                ...
                if (recentNoteStreakInterval < currentNoteStreakInterval)
                {
                    commonTrack.TrackView.ShowPerformanceText(
                        $"{currentNoteStreakInterval * intervalSize}-NOTE STREAK");
                }
            }
            ...
            int startIndex = 0)
        {
            ...
            for (newIndex = startIndex; newIndex < Chart.Count; newIndex++)
            {
                ...
                if ((note.time == lastNoteTime && chordsAreSingleNote) // Chords count as a single note
                    || note.time < phrase.time)
                {
                    // Skip notes that are before the phrase
                    continue;
                }
