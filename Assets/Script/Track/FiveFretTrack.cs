...
namespace YARG.PlayMode
{
    public class FiveFretTrack : AbstractTrack
    {
        ...
        private bool[] extendedSustain = new bool[]
        {
        ...
        private int[] allowedChordGhosts = new int[]
        {
            -1, -1, -1, -1, -1
        }; // -1 = not a chord; 0 = ghosted; 1 = ghost allowed
        ...
        private void UpdateInput()
        {
            ...
            if (strumLeniency > 0f && !strummed)
            {
                ...
                else
                {
                    ...
                    {
                        // Consume allowed overstrum as soon as it's "hit"
            ...
            {
                RemoveOldAllowedOverstrums();
                var overstrumForgiven =
                    IsOverstrumForgiven(
                        false); // Do NOT consume allowed overstrums; this is done in other parts of the code
            ...
            if (strumLeniencyEnded)
            {
                // Strum leniency ended, and suitable strummable note wasn't found; overstrum
                UpdateOverstrums();
            }
            ...
            else if (allowedOverstrums.Count > 0 && !chord[0].hopo && !chord[0].tap)
            {
                for (int i = 0; i < allowedOverstrums.Count; i++)
                {
                    ...
                    {
                        allowedOverstrums
                            .Clear(); // If latest strum is different from latest HO/PO, disallow overstrumming
                        break;
                    }
                    else
                    {
                ...
                for (int i = 0; i < frets.Length; i++)
                {
                    ...
                    else if (!contains && frets[i].IsPressed)
                    {
                        if (Constants.ANCHORING && Constants.ANCHOR_CHORD_HOPO &&
                            (chordList[0].hopo || chordList[0].tap) &&
                            !(strummed || strumLeniency > 0f || overstrumCheck) &&
                            i < chordList[0].fret)
                        {
        ...
        private void FretChangedAction(bool pressed, int fret)
        {
            ...
            if (SettingsManager.Settings.AntiGhosting.Data && allowedGhosts > 0 && pressed && hitChartIndex > 0)
            {
                ...
                if (Constants.ALLOW_DESC_GHOSTS)
                {
                    for (var i = 0; i < 5; i++)
                    {
                        ...
                        if (frets[i].IsPressed)
                        {
                            if (fret < i)
                            {
                                // Don't check ghosting if pressed fret is below currently held fret
                                checkGhosting = false;
                                break;
                            }
                        }
                ...
                if (checkGhosting)
                {
                    ...
                    if (checkGhosting)
                    {
                        if (nextNote.Count == 1 && fret != nextNote[0].fret)
                        {
                            // Hitting wrong button = ghosted = bad
                            allowedGhosts--;
                        }
                        ...
                        {
                            // If chord...
                            if (allowedChordGhosts[fret] == 1)
                            {
                                // Fret is part of chord, and hasn't been ghosted yet
            ...
            if (pressed)
            {
                ...
                if (!IsExtendedSustain())
                {
                    // Unless it's extended sustains
                    bool release = false;
                    //
                    for (int i = heldNotes.Count - 1; i >= 0; i--)
                    {
                        var heldNote = heldNotes[i];
                        if (heldNote.fret < 5 &&
                            (heldNote.fret == fret || (heldNotes.Count == 1 && fret < heldNote.fret)))
                        {
                            // Button press is valid
                            continue;
                        }
                        else
                        {
                            // Wrong button pressed; release all sustains
                            release = true;
                            break;
                        }
                    }
                    ...
                    if (release)
                    {
                        ...
                        for (int i = heldNotes.Count - 1; i >= 0; i--)
                        {
                            ...
                            if (heldNote.fret < 5) frets[heldNote.fret].StopAnimation(); // TEMP (remove check later)
                            if (heldNote.fret < 5)
                                frets[heldNote.fret].StopSustainParticles(); // TEMP (remove check later)
                            extendedSustain[heldNote.fret] = false;
                        }
        ...
        private string PrintFrets()
        {
            // Debug function; remove later?
            return "[" + (frets[0].IsPressed ? "G" : "") + (frets[1].IsPressed ? "R" : "") +
                (frets[2].IsPressed ? "Y" : "") + (frets[3].IsPressed ? "B" : "") + (frets[4].IsPressed ? "O" : "") +
                "]";
        }
        ...
        private bool ChordsOverlap(List<NoteInfo> chordList1, List<NoteInfo> chordList2)
        {
            ...
            {
                if (chord.length > 0.2f && chord.EndTime > chordList2[0].time)
                {
                    // If it's a sustain and overlaps with next note...
                    return true;
                }
            }
