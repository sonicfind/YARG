    ...
    public static class MiloParser
    {
        ...
        private static Dictionary<string, byte[]> ParseMiloForFiles(FrameworkFile file)
        {
            ...
            for (int i = 0; i < entryCount; i++)
            {
                ...
                MiloEntryNames[i] =
                    Encoding.UTF8.GetString(br.ReadBytes((int) MiloEntryNameCnt), 0, (int) MiloEntryNameCnt);
            }
            ...
            for (int i = 0; i < parentCnt; i++)
                br.ReadBytes((int) BinaryPrimitives.ReadUInt32BigEndian(br.ReadBytes(4))); // skip miloParents

            br.ReadBoolean(); // skip miloU10

            int childrenCnt = BinaryPrimitives.ReadInt32BigEndian(br.ReadBytes(4));
            for (int i = 0; i < childrenCnt; i++)
                br.ReadBytes((int) BinaryPrimitives.ReadUInt32BigEndian(br.ReadBytes(4))); // skip miloChildren
            ...
            for (int i = 0; i < childrenCnt; i++)
            {
                ...
                for (int k = 0; k < subEntryCount; k++)
                {
                    ...
                    subMiloEntryNames[k] = Encoding.UTF8.GetString(br.ReadBytes((int) subMiloEntryNameCnt), 0,
                        (int) subMiloEntryNameCnt);
                }
                ...
                if (subVersion == 28)
                {
                    ...
                    br.ReadBytes(
                        (int) BinaryPrimitives.ReadUInt32BigEndian(br.ReadBytes(4))); // skip subMiloSubnameCnt 
                    br.ReadBytes(8);                                                  // skip subMiloU5 and skipMiloU6
                }

                br.ReadBytes(BinaryPrimitives.ReadInt32BigEndian(br.ReadBytes(4)) * 48 +
                    9); // skip matrices and miloU7-9
                ...
                if (subChildrenCnt > 0)
                    throw new Exception("ERROR: should not have further children within this subdir!");

                byte[] subRemaining = br.ReadBytes(17);
                if (!FoundPadding(subRemaining.ToList()))
                    throw new Exception(
                        $"Expected padding AD-DE-AD-DE in remaining sub-bytes {BitConverter.ToString(subRemaining)}");
        ...
        private static readonly string[] AnimParts =
        {
            "lightpreset_interp", "lightpreset_keyframe_interp", "world_event", "spot_guitar", "spot_bass",
            "spot_drums", "spot_keyboard", "spot_vocal", "part2_sing", "part3_sing", "part4_sing", "postproc_interp",
            "shot_bg", "stagekit_fog"
        };
            ...
            foreach (var part in AnimParts)
            {
                ...
                if (offset != -1)
                {
                    ...
                    for (int i = 0; i < events; i++)
                    {
                        ...
                        if (eventLen > 0)
                            eventName = Encoding.UTF8.GetString(br.ReadBytes(eventLen), 0, eventLen);
                        else if (eventsList.Count == 0)
                            listIsActive = false;
                        else
                            eventName = eventsList.Last().name;
                        ...
                        long timeInTicks =
                            TimeConverter.ConvertFrom(new MetricTimeSpan((long) (timeAdd * 1000000)), tmap);
                    ...
                    if (part.EndsWith("_sing") || part.StartsWith("spot_"))
                    {
                        noteVal = part switch
                        {
                            ...
                            _ => throw new Exception(
                                $"Unknown singalong or spotlight event found at tick count {eventInstance.time}!"),
                        };
                        if (eventInstance.name.EndsWith("on"))
                        {
                            if (prevType == "note_on")
                            {
                                tempEvents.Add(new NoteOffEvent()
                                {
                                    NoteNumber = (SevenBitNumber) noteVal,
                                    Velocity = (SevenBitNumber) 0,
                                    DeltaTime = timeVal
                                });
                                tempEvents.Add(new NoteOnEvent()
                                {
                                    NoteNumber = (SevenBitNumber) noteVal,
                        ...
                        else if (eventInstance.name.EndsWith("off"))
                        {
                            ...
                            throw new Exception($"Unknown state event found at tick count {eventInstance.time}");
                    }
                    else if (part == "lightpreset_interp")
                        tempEvents.Add(new TextEvent($"[lighting ({eventInstance.name})]")
                        {
                            DeltaTime = timeVal
                        });
                    else if (part == "stagekit_fog")
                        tempEvents.Add(
                            new TextEvent($"[Fog{char.ToUpper(eventInstance.name[0]) + eventInstance.name[1..]}]")
                            {
                                DeltaTime = timeVal
                            });
                    else
                        tempEvents.Add(new TextEvent($"[{eventInstance.name}]")
                        {
                            DeltaTime = timeVal
                        });
        ...
        private static TrackChunk LipsyncToMidi(byte[] lipBytes, TempoMap tmap, string trackName)
        {
            ...
            foreach (var lip in lipsyncData)
            {
                if (lip.changes != 0)
                {
                    ...
                    {
                        if (y % 2 == 0)
                            visemeEdit = lip.byteArr[y];
                        else
                            visemeFrame[visemeEdit] = lip.byteArr[y];
                    }
                }
            ...
            var lipsyncTrack = new List<MidiEvent>
            {
                ...
                if (!Enumerable.SequenceEqual(prevFrame, visemeState[y]))
                {
                    ...
                    for (int i = 0; i < visemeState[y].Count; i++)
                    {
                        if (visemeState[y][i] != prevFrame[i])
                        {
                            ...
                            lipsyncTrack.Add(new TextEvent()
                            {
                                Text = textEvent, DeltaTime = timeVal
                            });
                            timeVal = 0;
                        }
                    }
