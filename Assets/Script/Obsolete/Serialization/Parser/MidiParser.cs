...
namespace YARG.Serialization.Parser
{
    public partial class MidiParser : AbstractParser
    {
        ...
        public override void Parse(YargChart chart)
        {
                ...
                foreach (var trackEvent in trackChunk.Events)
                {
                    ...
                    try
                    {
                        ...
                        if (trackName.Text.StartsWith("HARM") || trackName.Text.StartsWith("PART HARM"))
                        {
                            ...
                            {
                                chart.harmLyrics[harmIndex] =
                                    ParseRealLyrics(eventIR, trackChunk, harm1Chunk, tempo, harmIndex);
                            }
                            ...
                            case "PART DRUMS":
                                ...
                                else
                                {
                                    for (int i = 0; i < 5; i++)
                                    {
                                        ...
                                        chart.RealDrums[i] = ParseDrums(trackChunk, true, i, drumType,
                                            chart.GhDrums[i]);
                                    }
            ...
            {
                float startTime =
                    (float) TimeConverter.ConvertTo<MetricTimeSpan>(eventInfo.startTick, tempo).TotalSeconds;

                // If the event has length, do that too
                if (eventInfo.endTick.HasValue)
                {
                    float endTime = (float) TimeConverter.ConvertTo<MetricTimeSpan>(eventInfo.endTick.Value, tempo)
                        .TotalSeconds;
        ...
        private void ParseGlobalEvents(List<EventIR> eventIR, TrackChunk trackChunk)
        {
            ...
            foreach (var trackEvent in trackChunk.Events)
            {
                ...
                if (trackEvent is BaseTextEvent textEvent)
                {
                    ...
                    {
                        startTick = totalDelta, name = text
                    });
                }
            }
        }
        ...
        private void ParseStarpower(List<EventIR> eventIR, TrackChunk trackChunk, string instrument)
        {
            ...
            foreach (var trackEvent in trackChunk.Events)
            {
                ...
                else if (trackEvent is NoteOffEvent)
                {
                    ...
                    eventIR.Add(new EventIR
                    {
                        startTick = starPowerStart.Value, endTick = totalDelta, name = $"starpower_{instrument}"
                    });
                    starPowerStart = null;
                }
            }
        ...
        private void ParseSolo(List<EventIR> eventIR, TrackChunk trackChunk, string instrument, int soloOctave = 7)
        {
            ...
            foreach (var trackEvent in trackChunk.Events)
            {
                ...
                else if (trackEvent is NoteOffEvent)
                {
                    ...
                    eventIR.Add(new EventIR
                    {
                        startTick = soloStart.Value, endTick = totalDelta, name = $"solo_{instrument}"
                    });
                    soloStart = null;
                }
            }
        ...
        private void ParseDrumFills(List<EventIR> eventIR, TrackChunk trackChunk, string instrument)
        {
            ...
            foreach (var trackEvent in trackChunk.Events)
            {
                ...
                else if (trackEvent is NoteOffEvent)
                {
                    ...
                    eventIR.Add(new EventIR
                    {
                        startTick = fillStart.Value, endTick = totalDelta, name = $"fill_{instrument}"
                    });
                    fillStart = null;
                }
            }
